import "./widgetLg.css";

export default function WidgetLg() {
  const Button = ({ type }) => {
    return <button className={"widgetLgButton " + type}>{type}</button>;
  };
  return (
    <div className="widgetLg">
      <h3 className="widgetLgTitle">Latest Transactions</h3>
      <table className="widgetLgTable">
        <tr className="widgetLgTr">
          <th className="widgetLgTh">Customer</th>
          <th className="widgetLgTh">Date</th>
          <th className="widgetLgTh">Amount</th>
          <th className="widgetLgTh">Status</th>
        </tr>
        <tr className="widgetLgTr">
          <td className="widgetLgUser">
            <img
              src="https://burst.shopifycdn.com/photos/man-takes-time-to-ponder.jpg?width=925&format=pjpg&exif=1&iptc=1"
              alt=""
              className="widgetLgImg"
            />
            <span className="widgetLgName">Eliteboss</span>
          </td>
          <td className="widgetLgDate">2 Jun 2021</td>
          <td className="widgetLgAmount">$500.33</td>
          <td className="widgetLgStatus">
            <Button type="Approved" />
          </td>
        </tr>
        <tr className="widgetLgTr">
          <td className="widgetLgUser">
            <img
              src="https://burst.shopifycdn.com/photos/man-smiles-over-coffee-in-cafe.jpg?width=925&format=pjpg&exif=1&iptc=1"
              alt=""
              className="widgetLgImg"
            />
            <span className="widgetLgName">Justin Ojak</span>
          </td>
          <td className="widgetLgDate">5 Jun 2021</td>
          <td className="widgetLgAmount">$960.00</td>
          <td className="widgetLgStatus">
            <Button type="Approved" />
          </td>
        </tr>
        <tr className="widgetLgTr">
          <td className="widgetLgUser">
            <img
              src="https://burst.shopifycdn.com/photos/urban-mens-fashion.jpg?width=925&format=pjpg&exif=1&iptc=1"
              alt=""
              className="widgetLgImg"
            />
            <span className="widgetLgName">Mark Stone</span>
          </td>
          <td className="widgetLgDate">7 Jun 2021</td>
          <td className="widgetLgAmount">$400.23</td>
          <td className="widgetLgStatus">
            <Button type="Declined" />
          </td>
        </tr>
        <tr className="widgetLgTr">
          <td className="widgetLgUser">
            <img
              src="https://burst.shopifycdn.com/photos/male-dentist.jpg?width=925&format=pjpg&exif=1&iptc=1"
              alt=""
              className="widgetLgImg"
            />
            <span className="widgetLgName">Jeff Omark</span>
          </td>
          <td className="widgetLgDate">10 feb 2021</td>
          <td className="widgetLgAmount">$1000.00</td>
          <td className="widgetLgStatus">
            <Button type="Declined" />
          </td>
        </tr>
        <tr className="widgetLgTr">
          <td className="widgetLgUser">
            <img
              src="https://burst.shopifycdn.com/photos/stylish-man-in-bow-tie.jpg?width=925&format=pjpg&exif=1&iptc=1"
              alt=""
              className="widgetLgImg"
            />
            <span className="widgetLgName">Dammy Omar</span>
          </td>
          <td className="widgetLgDate">4 mar 2021</td>
          <td className="widgetLgAmount">$750.33</td>
          <td className="widgetLgStatus">
            <Button type="Pending" />
          </td>
        </tr>
      </table>
    </div>
  );
}
